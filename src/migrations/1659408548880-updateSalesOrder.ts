...
export class updateSalesOrder1659408548880 implements MigrationInterface {
    name = 'updateSalesOrder1659408548880';
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`sales_order\` CHANGE \`phone_number\` \`phone_number\` varchar(11) NOT NULL`
        );
        await queryRunner.query(
            `ALTER TABLE \`sales_order\` CHANGE \`customer_name\` \`customer_name\` varchar(50) NOT NULL`
        );
        await queryRunner.query(
